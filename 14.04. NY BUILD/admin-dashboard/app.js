...
const state = {
  ...
  facilityRiskSummary: { ekstrem: 0, "hoy": 0, moderat: 0, lav: 0, total: 0 },
...
function initializeElements() {
  elements = {
    ...
    facilityRiskhoyCount: document.getElementById("facilityRiskhoyCount"),
  ...
  if (!elements.facilityRiskhoyCount) elements.facilityRiskhoyCount = getById("fr-hoy");
...
async function showRiskFactorModal(pred) {
  ...
  if (hasDisease) {
    ...
    explanation = `Dette anlegget kan SPRE ${primaryDisease} til andre anlegg i området`;
    targetInfo = `<div style="padding: 12px; background: #fff; border-radius: 6px; border-left: 4px solid #dc2626; margin-top: 12px;">
      <div style="font-weight: 700; color: #7f1d1d; margin-bottom: 6px;">⚠️ SMITTERISIKO FOR ANDRE ANLEGG</div>
      <div style="color: #1f2937; font-size: 0.95rem; line-height: 1.6;">
        Anlegg i nærheten kan bli smittet via båttrafikk og havstrømmer. 
    ...
    mainMessage = `${facilityName} ER FRISKT (ikke smittet ennå)`;
    ...
    targetInfo = `<div style="padding: 14px; background: white; border-radius: 6px; border: 2px solid #fca5a5; margin-top: 12px;">
      <div style="font-weight: 700; color: #7f1d1d; margin-bottom: 8px; font-size: 0.95rem;">🔻 SMITTEKILDE</div>
      <div style="padding: 10px; background: #fef2f2; border-radius: 4px; margin-bottom: 8px;">
        ...
        <div style="color: #1f2937; font-size: 0.85rem; margin-top: 4px;">📏 ${pred.distance_to_nearest_infected?.toFixed(1) || "?"} km unna</div>
      </div>
      <div style="color: #1f2937; font-size: 0.95rem; line-height: 1.5;">
        ...
        via båter som besøker begge anlegg eller via havstrømmer. 
    ...
    explanation = `Ingen smittede anlegg i umiddelbar nærhet`;
    targetInfo = `<div style="padding: 12px; background: #f0fdf4; border-radius: 6px; border-left: 4px solid #22c55e; margin-top: 12px;">
      <div style="font-weight: 600; color: #15803d;">✓ Ingen identifisert smittekilde</div>
      <div style="color: #1f2937; font-size: 0.9rem; margin-top: 6px;">Ingen smittede anlegg funnet i nærheten</div>
    </div>`;
  }
  
  const modeDisplay = hasDisease
    ? {
        title: "🔴 OVERFØRINGSRISIKO",
  ...
  const riskBreakdown = `
    <div class="modal-card-item" style="background: #f9fafb; border-left: 4px solid #6b7280; padding: 14px; margin-bottom: 16px;">
      <div style="font-weight: 700; color: #111827; margin-bottom: 12px; font-size: 0.95rem;">📊 RISIKOFAKTOR-BIDRAG (Totalt 100 pts)</div>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
        <div>
          ${contribBar('📏 Avstand', distContrib, 50)}
          ${contribBar('🌊 Havstrøm', oceanContrib, 50)}
        </div>
        <div>
          ${contribBar('🦠 Sykdomstype', diseaseContrib, 50)}
          ${contribBar('🚢 Båttrafikk', vesselContrib, 50)}
        </div>
      </div>
      <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #d1d5db; font-size: 0.85rem; color: #6b7280;">
        <strong>Hovedårsaker:</strong> ${(pred.risk_drivers || []).map(d => {
          if (d === 'distance') return '📏 Avstand';
          if (d === 'ocean_current') return '🌊 Havstrøm';
          if (d === 'disease') return '🦠 Sykdomstype';
          if (d === 'vessels') return '🚢 Båter';
          return d;
      ...
      <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #d1d5db; font-size: 0.75rem; color: #9ca3af;">
        💡 <strong>Hva betyr denne scoren?</strong> En score på ${riskScore} poeng indikerer risikoen basert på avstand, havstrøm-retning og sykdomstype. Dette er et estimat, ikke en presise sannsynlighet.
      </div>
    </div>
  `;
  ...
  const facilityInfo = `
    <div class="modal-card-item">
      <div class="label" style="color: #111827; font-weight: 600;">📍 Anleggsinformasjon</div>
      <div style="margin-top: 8px;">
        ...
        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 6px;">📍 ${pred.latitude?.toFixed(4)}, ${pred.longitude?.toFixed(4)}</div>
        <div style="font-size: 0.85rem; color: #6b7280;">📅 ${pred.prediction_date?.split('T')[0] || "--"}</div>
      </div>
    </div>
  `;
  ...
  const boatInfo = totalBoats > 0 ? `
    <div class="modal-card-item">
      <div class="label" style="color: #111827; font-weight: 600;">🚢 Båttrafikk (siste 7 dager)</div>
      <div style="display: flex; gap: 10px; margin-top: 10px;">
        <div style="flex: 1; padding: 14px; background: #f0fdf4; border-left: 4px solid #22c55e; border-radius: 6px;">
          <div style="font-size: 0.85rem; color: #374151; margin-bottom: 4px; font-weight: 600;">Normale båter</div>
        ...
        <div style="flex: 1; padding: 14px; background: #fef2f2; border-left: 4px solid #dc2626; border-radius: 6px;">
          <div style="font-size: 0.85rem; color: #374151; margin-bottom: 4px; font-weight: 600;">⚠️ Karantene-båter</div>
          <div style="font-weight: 700; font-size: 1.4rem; color: #991b1b;">${quarantineBoats}</div>
          <div style="font-size: 0.8rem; color: #7f1d1d; margin-top: 4px; font-weight: 600;">3x hoyere risiko</div>
        </div>
        ` : ""}
      </div>
      <div style="margin-top: 10px; font-size: 0.9rem; color: #1f2937; font-weight: 500;">
        Totalt: ${totalBoats} båt${totalBoats !== 1 ? 'er' : ''} har besøkt anlegget
      </div>
    </div>
  ` : `
    <div class="modal-card-item">
      <div class="label" style="color: #111827; font-weight: 600;">🚢 Båttrafikk (siste 7 dager)</div>
      <div style="font-size: 0.95rem; color: #374151; margin-top: 8px; font-weight: 500;">Ingen båter registrert</div>
    </div>
  `;
      ...
      if (vesselsForFacility.length > 0) {
            ...
            <div style="padding: 12px; background: #f9fafb; border-radius: 6px; border-left: 4px solid ${riskColor}; margin-bottom: 10px;">
              <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                <div>
                  ...
                  <div style="font-size: 0.8rem; color: #6b7280; margin-top: 2px;">📅 Siste besøk: ${visitDate} • ⏱️ ${visitDuration}</div>
                </div>
              ...
              ${otherFacilities.length > 0 ? `
                <div style="font-size: 0.85rem; color: #374151; margin-top: 8px;">
                  <strong>Andre anlegg besøkt (7d):</strong>
        ...
        vesselVisitsSection = `
          <div class="modal-card-item" style="border: 2px solid #3b82f6; background: #eff6ff;">
            <div class="label" style="color: #1e40af; font-weight: 600;">🚢 Båter som har besøkt dette anlegget (siste 7 dager)</div>
            <div style="font-size: 0.85rem; color: #1f2937; margin-top: 6px; margin-bottom: 12px;">
              Viser ${Math.min(vesselsForFacility.length, 10)} av ${vesselsForFacility.length} båt${vesselsForFacility.length !== 1 ? 'er' : ''} som har besøkt ${facilityName}
            </div>
  ...
  const oceanInfo = oceanRisk !== undefined && oceanRisk > 0
    ? `
      <div class="modal-card-item" style="background: #eff6ff; border-left: 4px solid #2563eb; padding: 14px;">
        <div class="label" style="color: #1e40af; font-weight: 600;">🌊 Havstrøm (NorKyst-800)</div>
        <div style="font-weight: 700; font-size: 1.2rem; color: #1e40af; margin-top: 6px;">${(oceanRisk).toFixed(1)}/100</div>
        <div style="font-size: 0.85rem; color: #1f2937; margin-top: 4px; font-weight: 500;">Strømretning-analyse aktiv</div>
      </div>
    `
    : "";
  ...
  body.innerHTML = `
    ...
    ${oceanInfo}
    <div class="modal-card-item">
      <div class="label" style="color: #111827; font-weight: 600;">📊 Hovedfaktorer som driver risiko</div>
      <div style="font-size: 0.95rem; margin-top: 10px; line-height: 1.7; color: #1f2937; font-weight: 500;">${driversText}</div>
    </div>
    <div class="modal-card-item">
      <div class="label" style="color: #111827; font-weight: 600;">📈 Teknisk faktor-oversikt</div>
...
async function showTransmissionModal(pred) {
  ...
  const sourcesList = nearbySources.length > 0 
    ? nearbySources.map((s, idx) => `<div style="padding:8px; background:${idx === 0 ? '#fee2e2' : 'white'}; border-radius:4px; margin-bottom:6px; border-left:3px solid ${idx === 0 ? '#dc2626' : '#f97316'};">
        <div style="font-weight:600; color:#1f2937;">${idx === 0 ? '🔴 ' : '🟠 '}${s.name}</div>
        <div style="color:#6b7280; font-size:0.85rem;">📍 ${s.distance} km unna</div>
        ${s.latitude && s.longitude ? `<div style="color:#9ca3af; font-size:0.75rem;">(${s.latitude?.toFixed(2)}, ${s.longitude?.toFixed(2)})</div>` : ''}
        ${s.diseases && s.diseases.length > 0 ? `<div style="color:#dc2626; font-size:0.8rem; font-weight:600;">🦠 ${s.diseases.join(', ')}</div>` : ''}
      </div>`).join('')
  ...
  let transmissionHTML = `
    ...
    <div style="padding: 16px; background: linear-gradient(135deg, #dc2626 0%, #f97316 100%); border-radius: 8px; margin-bottom: 16px; box-shadow: 0 4px 6px rgba(220, 38, 38, 0.3);">
      <div style="font-weight: 700; color: white; font-size: 1.2rem; margin-bottom: 6px;">⚠️ SMITTEKLYNGE DETEKTERT</div>
      <div style="color: white; font-size: 0.95rem; line-height: 1.6;">
        ${nearbySources.length} smittede anlegg innenfor 50 km radius. 
        <strong>Risiko økt med ${((pred.cluster_multiplier - 1) * 100).toFixed(0)}% pga. flere smittekilder.</strong>
      </div>
    </div>
    ` : ''}
    <div style="padding: 16px; background: #fff3e0; border-radius: 8px; border-left: 5px solid #f97316; margin-bottom: 20px;">
      <div style="font-weight: 700; color: #7c2d12; font-size: 1.1rem; margin-bottom: 8px;">🧬 Smittekjede-analyse</div>
      <div style="color: #1f2937; font-size: 0.95rem; line-height: 1.6;">
        <strong class="text-em">${facilityName}</strong> har risikoscore <strong class="text-em">${riskScore} poeng</strong> for å bli smittet 
    ...
    <div style="margin-bottom: 24px;">
      <div style="font-weight: 700; color: #111827; margin-bottom: 12px; font-size: 1rem;">📊 SMITTEFLYT FRA KLYNGE</div>
      
      <!-- Source Facility Box -->
      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
        <div style="flex: 1; padding: 14px; background: #fee2e2; border: 2px solid #dc2626; border-radius: 8px;">
          <div style="font-weight: 700; color: #7f1d1d; font-size: 1rem;">🔴 SMITTEKILDER</div>
          ${sourcesList}
          <div style="color: #6b7280; font-size: 0.85rem; margin-top: 4px;">📍 ${sourceDistance} km unna</div>
          <div style="color: #dc2626; font-weight: 600; font-size: 0.85rem; margin-top: 6px;">🦠 ${primaryDisease} bekreftet</div>
        </div>
      </div>

      <!-- Arrow -->
      <div style="text-align: center; font-size: 1.5rem; color: #f97316; margin-bottom: 16px; letter-spacing: 2px;">⬇ ⬇ ⬇</div>
      ...
      <div style="padding: 14px; background: #f3e8ff; border: 2px solid #a855f7; border-radius: 8px; margin-bottom: 16px;">
        <div style="font-weight: 700; color: #6b21a8; font-size: 0.95rem; margin-bottom: 8px;">🚢 MULIGE SMITTEVEKTORER</div>
        <div style="color: #1f2937; font-size: 0.9rem; line-height: 1.6;">
          <div style="margin-bottom: 8px;">
            <strong>Båter:</strong> Kommersielle båter og Service-båter som besøker både smittekilder og <strong>${facilityName}</strong> kan transportere virus.
          </div>
          <div style="margin-bottom: 8px;">
            <strong>Hydrologi:</strong> Havstrømmer fra klyngen av smittede anlegg (innenfor 50 km) kan transportere virus direkte.
          </div>
          <div>
      ...
      <div style="padding: 14px; background: #fef3c7; border: 2px solid #f59e0b; border-radius: 8px;">
        <div style="font-weight: 700; color: #92400e; font-size: 1rem;">🟠 RISIKOUTSATT ANLEGG</div>
    ...
    <div style="padding: 14px; background: #f0f9ff; border-left: 4px solid #0284c7; border-radius: 6px; margin-bottom: 20px;">
      <div style="font-weight: 700; color: #0c4a6e; font-size: 0.95rem; margin-bottom: 10px;">⚡ KRITISKE RISIKOFAKTORER</div>
      <ul style="margin: 0; padding-left: 20px; color: #1f2937; font-size: 0.9rem; line-height: 1.8;">
        <li><strong>Smitteklynge:</strong> ${nearbySources.length} anlegg innenfor 50 km${nearbySources.length > 1 ? ` (risiko økt med ${((pred.cluster_multiplier - 1) * 100).toFixed(0)}% pga. flere kilder)` : ''}</li>
        <li><strong>Avstand:</strong> Nærmeste smittekilde: ${sourceDistance} km</li>
        <li><strong>Båttrafikk:</strong> (se båt-vektor-data under)</li>
        <li><strong>Sykdomstype:</strong> ${primaryDisease} – (overføringshastighet: hoy)</li>
        <li><strong>Tidsvindu:</strong> Virus kan overleve 5-7 dager i lavtemperatur-miljø</li>
    ...
    <div style="padding: 14px; background: #dcfce7; border-left: 4px solid #16a34a; border-radius: 6px; margin-bottom: 20px;">
      <div style="font-weight: 700; color: #166534; font-size: 0.95rem; margin-bottom: 10px;">✓ TILTAK</div>
      <ul style="margin: 0; padding-left: 20px; color: #1f2937; font-size: 0.9rem; line-height: 1.8;">
        <li><strong>Båtkarantene:</strong> Isoler båter som besøker smitteklyngen</li>
        <li><strong>Desinfeksjon:</strong> Krev desinfeksjon før besøk hos ${facilityName}</li>
        <li><strong>Overvåking:</strong> Øk testfrekvens de neste 30 dagene</li>
        <li><strong>Redusert trafikk:</strong> Limit båt-bevegelse mellom sonene</li>
      </ul>
    </div>

    <!-- Boat Vector Details (if available) -->
    <div style="padding: 14px; background: #f3f4f6; border-radius: 6px; border: 1px solid #d1d5db;">
      <div style="font-weight: 700; color: #111827; font-size: 0.95rem; margin-bottom: 10px;">🚢 BÅTER MED KONTAKT TIL SMITTEKILDENE</div>
      <div style="color: #6b7280; font-size: 0.9rem; margin-bottom: 10px;">
        Disse båtene har besøkt smitteklyngen eller ligger i nærheten og kan være vektorer:
      </div>
      <div id="boatVectorsList" style="color: #6b7280; font-size: 0.85rem; font-style: italic;">
        (Båt-data lastes fra API...)
      </div>
    </div>
  `;
      ...
      const boatHTML = vesselData.vessels.slice(0, 5).map((vessel, idx) => `
        <div style="padding: 10px; background: white; border-radius: 4px; margin-bottom: 8px; border-left: 3px solid #0284c7;">
          ...
          <div style="color: #6b7280; font-size: 0.85rem; margin-top: 3px;">
            📍 Avstand: ${(vessel.distance_km ?? "?").toFixed(1)} km
            • ⏱️ Sist besøk: ${vessel.last_visit ? new Date(vessel.last_visit).toLocaleDateString('no-NO') : "Unknown"}
            • 🟠 Risiko: ${vessel.risk_level}
          </div>
        </div>
      `).join('');
      
      document.getElementById("boatVectorsList").innerHTML = boatHTML || "<div style='color: #9ca3af;'>Ingen båt-aktivitet detektert</div>";
    }
  } catch (err) {
    console.error('Failed to load boat vector data:', err);
    document.getElementById("boatVectorsList").innerHTML = "<div style='color: #ef4444;'>Kunne ikke laste båt-data</div>";
  }
...
async function apiFetch(path, options = {}) {
  ...
  // Safety timeout: increased to 300s (5 minutes) for slow API endpoints
  const timeoutMs = path.includes('at-risk-facilities') ? 300 * 1000 : 5 * 60 * 1000;
...
async function loadDecisionLine() {
  const container = document.getElementById("decisionLineContainer");
  const timeEl = document.getElementById("decisionLineTime");
  
  if (!container) return;
  
  try {
    timeEl.textContent = "laster...";
    const response = await apiFetch("/api/admin/decision-line");
    
    if (!response || !response.all_actions) {
      container.innerHTML = '<div style="text-align: center; padding: 32px; color: var(--muted);">Ingen prioriterte tiltak akkurat n�</div>';
      timeEl.textContent = "oppdatert n�";
      return;
    }
    
    const actions = response.all_actions || [];
    const now = new Date();
    const timeStr = now.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
    timeEl.textContent = `oppdatert ${timeStr}`;
    
    if (actions.length === 0) {
      container.innerHTML = '<div style="text-align: center; padding: 32px; color: var(--muted);">? Ingen kritiske tiltak for �yeblikket</div>';
      return;
    }
    
    const html = actions.map((action, idx) => {
      const severityColor = action.severity === "Kritisk" ? "#ef4444" : 
                           action.severity === "H�y" ? "#fbbf24" : 
                           action.severity === "Moderat" ? "#8b5cf6" : "#22c55e";
      
      const severityIcon = action.severity === "Kritisk" ? "??" : 
                          action.severity === "H�y" ? "??" : 
                          action.severity === "Moderat" ? "??" : "??";
      
      const deadline = action.hours_until_deadline !== undefined && action.hours_until_deadline > 0
        ? `${Math.round(action.hours_until_deadline)}h` 
        : action.deadline || "�";
      ...
      return `
        <div style="
          ...
          align-items: start;
        ">
          <div style="font-size: 2rem; line-height: 1;">${severityIcon}</div>
          <div>
            <div style="font-weight: 700; color: ${severityColor}; margin-bottom: 2px;">${action.title}</div>
            <div style="font-size: 0.9rem; color: var(--muted); margin-bottom: 8px;"><strong>Hva:</strong> ${action.what}</div>
            <div style="font-size: 0.9rem; color: var(--muted); margin-bottom: 8px;"><strong>Hvorfor:</strong> ${action.why}</div>
            ${action.recommendation ? `<div style="font-size: 0.85rem; background: rgba(255,255,255,0.05); padding: 8px; border-radius: 3px; border-left: 2px solid ${severityColor}; color: var(--accent);">${action.recommendation}</div>` : ""}
            ${action.affected_facilities ? `
              <div style="font-size: 0.85rem; margin-top: 8px; color: var(--muted);">
                <strong>Ber�rte anlegg:</strong> ${action.affected_facilities.map(f => `${f.name} (${f.code})`).join(", ")}
              </div>
            ` : ""}
          </div>
          <div style="text-align: right;">
            <div style="font-weight: 700; color: ${severityColor}; font-size: 1.2rem;">${deadline}</div>
            <div style="font-size: 0.75rem; color: var(--muted); margin-top: 4px;">frist</div>
          </div>
        </div>
      `;
    }).join("");
    ...
    console.error("Error loading decision line:", error);
    container.innerHTML = `<div style="text-align: center; padding: 32px; color: var(--warning);">?? Kunne ikke laste prioriterte tiltak</div>`;
    timeEl.textContent = "feil";
  }
}
...
async function loadFacilityRisk() {
  ...
  try {
    ...
    elements.facilityRiskhoyCount.textContent = formatNumber(data.risk_summary?.hoy || 0);
...
async function loadVesselRisk() {
  ...
  try {
    ...
    // Use API summary counts (API returns ekstrem_risk_vessels, hoy_risk_vessels, moderat_risk_vessels)
    const ekstremCount = data.ekstrem_risk_vessels || 0;
    const hoyCount = data.hoy_risk_vessels || 0;
    ...
    elements.vesselRiskHighCount.textContent = formatNumber(ekstremCount + hoyCount);
...
function ensureCorrelationDetailModal() {
  ...
  if (notifyBtn) {
    notifyBtn.addEventListener("click", () => {
      ...
      alert(`📧 Notify: Simulated email sent to ${facilityName} (${facilityEmail}).`);
...
function ensureAdminCorrelationLayout() {
  ...
  adminData.innerHTML = `
    <div style="margin-bottom:12px;padding:10px 12px;border:1px solid rgba(255,255,255,0.08);border-radius:8px;background:#0b1424;color:#9ca3af;font-size:0.88rem;">
      ...
      Nettverket viser koblinger mellom båter og anlegg i smittesoner. Rader med hoy risiko betyr nylig kontakt eller hoy sannsynlighet for videre smittespredning.
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:8px;margin-bottom:12px;">
      ...
      <div style="padding:10px;border:1px solid rgba(255,255,255,0.08);border-radius:8px;background:#0b1424;"><div style="color:#9ca3af;font-size:0.8rem;">hoy risiko båter</div><div id="correlationHighBoats" style="color:#e5e7eb;font-weight:700;">0</div></div>
    ...
    contentEl.innerHTML = `
      <div style="margin-bottom:16px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
          ...
          <div style="padding:10px;background:#111827;border-radius:6px;">
            <div style="color:#9ca3af;font-size:0.75rem;">hoy risiko</div>
...
async function loadAdmin() {
  ...
  try {
    ...
    if (!correlationData || !correlationData.summary || correlationData.summary.total_correlations === 0) {
      document.getElementById("admin-data").innerHTML = `
        <div style="padding: 2rem; text-align: center; color: #10b981; font-size: 1rem; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%;">
          ...
          <div><strong>Great news!</strong> Ingen hoyrisiko-koblinger akkurat nå.</div>
    ...
    document.getElementById("admin-data").innerHTML = `<div style="padding: 2rem; text-align: center; color: #ef4444;">❌ Feil ved lasting av risiko-nettverk: ${error.message}</div>`;
  }
}
...
function renderVesselRisk() {
  ...
  const hasMeaningfulFacilityMovement = (vesselData) => {
    const visits = dedupeVisitsForDisplay(vesselData?.visits || []);
    if (!Array.isArray(visits) || visits.length < 2) return false;
    ...
    const labels = visits
      ...
      .map(v => String(v.facility_name || v.facility_code || '').trim().toLowerCase())
      .filter(label => label.length > 0 && !label.includes('unknown') && !label.includes('ukjent'));

    if (labels.length < 2) return false;
    return new Set(labels).size >= 2;
  };

  // Update summary with chain count (requires actual movement between different facilities)
  const chainCount = filtered.filter(v => v.has_48h_chain && hasMeaningfulFacilityMovement(v)).length;
  elements.vesselRiskChainCount.textContent = formatNumber(chainCount);
  ...
  const tableRows = filtered.map(vessel => {
    ...
    const has48hChain = vessel.has_48h_chain && hasMeaningfulFacilityMovement(vessel);
    const quarantineAnalysis = vessel?.quarantine_analysis || {};
    const quarantineStatus = quarantineAnalysis.quarantine_status || 'NONE';
    const hasQuarantineBreach = quarantineAnalysis.has_quarantine_breach === true || quarantineStatus === 'QUARANTINE_BREACH';
    const hasSameZoneTransfer = quarantineAnalysis.has_same_zone_transfer === true || quarantineStatus === 'SAME_ZONE_TRANSFER';
    const hasChainOnly = quarantineAnalysis.has_chain_only === true || quarantineStatus === 'CHAIN_ONLY' || (has48hChain && !hasQuarantineBreach);
    ...
    let rowStyle = '';
    if (hasQuarantineBreach) {
      const breachDetails = quarantineAnalysis?.breach_details;
      let breachText = `BRUDD${shortestGapHours !== null ? ` (${shortestGapHours.toFixed(1)}t)` : ' (<48t)'}`;
      ...
      if (breachDetails) {
        ...
        breachText += `<br><span style="font-size: 0.8rem; font-weight: 500;">Fra ${source} ? ${destination} (${hoursText}t)</span>`;
      }
      
      quarantineDisplay = `<span style="color: #dc2626; font-weight: 700;">${breachText}</span>`;
      rowStyle = " style='background: rgba(220,38,38,0.08);'";
    } else if (hasSameZoneTransfer) {
      const transfer = quarantineAnalysis?.same_zone_transfer_details;
      let text = 'SAMME SONE';
      if (transfer && Number.isFinite(Number(transfer.hours_after_infection))) {
        text += ` (${Number(transfer.hours_after_infection).toFixed(1)}t)`;
      }
      quarantineDisplay = `<span style="color: #f59e0b; font-weight: 700;">${text}</span>`;
      rowStyle = " style='background: rgba(245,158,11,0.09);'";
    } else if (hasChainOnly) {
      quarantineDisplay = '<span style="color: #f59e0b; font-weight: 600;">KJEDE</span>';
    } else {
      quarantineDisplay = '<span style="color: #16a34a; font-weight: 600;">OK</span>';
    }
    ...
    let riskBadges = '';
    if (riskZoneCount > 0) {
      riskBadges += `<span style="background: #f97316; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem; margin-right: 4px; display: inline-block;">Vernesone (${riskZoneCount})</span>`;
    }
    if (near10kmCount > 0) {
    ...
    return `<tr${rowStyle}>
      ...
      <td><button class="btn-view-details" data-mmsi="${vessel.mmsi}" style="padding: 4px 12px; background: var(--accent); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.85rem;">Vis</button></td>
    </tr>`;
  }).join('');
...
function showVesselDetail(vessel) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div style="max-width: 980px; margin: 0 auto; background: var(--panel); border: 1px solid rgba(255,255,255,0.12); border-radius: 12px; padding: 20px; position: relative;">
          <button id="closeVesselModal" aria-label="Lukk" style="position:absolute; top:12px; right:12px; background:transparent; border:none; color:var(--ink); font-size:1.3rem; cursor:pointer;">�</button>
  ...
  const quarantineAnalysis = vessel.quarantine_analysis || {};
  const hasMeaningfulFacilityMovement = (() => {
    ...
    return labels.length >= 2 && new Set(labels).size >= 2;
  })();
  ...
  const hasContactChain = (vessel?.has_48h_chain === true) && hasMeaningfulFacilityMovement;
  ...
  const sameZoneTransferDetails = quarantineAnalysis.same_zone_transfer_details || null;

  const isUnknownFacilityLabel = (value) => {
    if (value === null || value === undefined) return true;
    const normalized = String(value).trim().toLowerCase();
    return normalized.length === 0 || normalized.includes('unknown') || normalized.includes('ukjent');
  };
  ...
  const breachNarrativeHTML = hasQuarantineBreach && breachDetails
    ? (() => {
        ...
        if (isUnknownFacilityLabel(source) || isUnknownFacilityLabel(destination)) {
          return '';
        }
        const hours = Number(breachDetails.hours_after_infection);
        ...
        return `
          <div style="margin: 14px 0 18px 0; padding: 14px; background: rgba(220,38,38,0.12); border-left: 4px solid #dc2626; border-radius: 8px;">
            <div style="font-weight: 700; color: #fecaca; margin-bottom: 6px;">Karantene-brudd oppdaget</div>
  ...
  const sameZoneNarrativeHTML = hasSameZoneTransfer && sameZoneTransferDetails
    ? (() => {
        const source = sameZoneTransferDetails.infected_source || 'Ukjent kilde';
        const destination = sameZoneTransferDetails.facility_name || 'ukjent anlegg';
        if (isUnknownFacilityLabel(source) || isUnknownFacilityLabel(destination)) {
          return '';
        }
        const hours = Number(sameZoneTransferDetails.hours_after_infection);
        const hoursText = Number.isFinite(hours) ? hours.toFixed(1) : 'ukjent';
        return `
          <div style="margin: 14px 0 18px 0; padding: 14px; background: rgba(245,158,11,0.12); border-left: 4px solid #f59e0b; border-radius: 8px;">
            <div style="font-weight: 700; color: #fde68a; margin-bottom: 6px;">Samme-sone overgang (ikke karantenebrudd)</div>
            <div style="color: #fef3c7; font-size: 0.95rem; line-height: 1.45;">
              Fra <strong>${source}</strong> til <strong>${destination}</strong> innen <strong>${hoursText} timer</strong>.
            </div>
          </div>
        `;
  ...
  const visits = dedupeVisitsForDisplay(vessel.visits || []);
  const verifiableVisits = visits.filter((visit) => {
    const label = visit.facility_name || visit.facility_code;
    return !isUnknownFacilityLabel(label);
  });
  ...
  if (visitChain.length > 0) {
    timelineHTML = `
      <div style="margin: 20px 0;">
        <h3 style="color: var(--accent); margin-bottom: 15px;">48-timers bes�kslinje (siste 5 bes�k)</h3>
        <div style="position: relative; padding-left: 20px;">
          ...
          ${visitChain.map((visit) => {
            ...
            const riskColor = visit.risk_level === 'ekstrem' || visit.risk_level === 'hoy' ? '#dc2626' : 
              ...
              <div style="position: relative; margin-bottom: 20px; padding-left: 20px;">
                ...
                <div style="background: rgba(255,255,255,0.05); padding: 12px; border-radius: 8px; border-left: 3px solid ${riskColor};">
                  <div style="font-weight: 600; color: var(--ink);">
                    ...
                    ${isInfected ? '<span style="color: #dc2626; margin-left: 8px;">SMITTET</span>' : ''}
                  </div>
                  <div style="color: var(--muted); font-size: 0.85rem; margin-top: 4px;">
                    ${visit.hours_ago.toFixed(1)} timer siden � 
                    Risiko: ${(visit.risk_level || 'ukjent').toUpperCase()}
                  </div>
                </div>
                </div>
              </div>
  ...
  const allVisitsHTML = `
    <div style="margin: 20px 0;">
      <h3 style="color: var(--accent); margin-bottom: 15px;">Alle anleggsbes�k (siste 7 dager)</h3>
      <div style="display: grid; gap: 10px;">
        ${verifiableVisits.map(visit => {
          ...
          return `
            <div style="padding: 12px; background: rgba(255,255,255,0.03); border-radius: 8px; border-left: 4px solid ${riskColor};">
              ...
              <div style="color: var(--muted); font-size: 0.85rem; margin-top: 4px;">
                ${visitDate} � ${visit.duration_minutes || 0} min � 
                Risiko: ${(visit.risk_level || 'unknown').toUpperCase()}
                ${visit.distance_meters ? ` � ${(visit.distance_meters / 1000).toFixed(2)} km` : ''}
              </div>
            </div>
          `;
        }).join('')}
  ...
  const modalContent = `
    <h2 style="color: var(--accent); margin-bottom: 20px;">
      B�t: ${vessel.vessel_name || `Vessel ${vessel.mmsi}`}
    </h2>
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
      ...
      <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
        <div style="color: var(--muted); font-size: 0.85rem;">Risikoniv�</div>
        <div style="color: var(--ink); font-weight: 600; font-size: 1.2rem; text-transform: uppercase;">${vessel.highest_risk_level || 'lav'}</div>
      </div>
      <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
        <div style="color: var(--muted); font-size: 0.85rem;">Bes�kskategorier</div>
        <div style="margin-top: 8px;">
          ${(() => {
            const visits = verifiableVisits || [];
            ...
            let badges = '';
            if (categories.has('infected')) badges += '<span style="background: #dc2626; color: white; padding: 3px 8px; border-radius: 4px; font-size: 0.75rem; margin-right: 4px; display: inline-block; margin-bottom: 4px;">Smittet</span>';
            if (categories.has('risk_zone')) badges += '<span style="background: #f97316; color: white; padding: 3px 8px; border-radius: 4px; font-size: 0.75rem; margin-right: 4px; display: inline-block; margin-bottom: 4px;">Risikosone</span>';
            if (categories.has('near_10km')) badges += '<span style="background: #eab308; color: white; padding: 3px 8px; border-radius: 4px; font-size: 0.75rem; display: inline-block; margin-bottom: 4px;">&lt;10km</span>';
            return badges || '<span style="color: #6b7280; font-size: 0.85rem;">No categories</span>';
          })()}
        </div>
      </div>
      <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
        <div style="color: var(--muted); font-size: 0.85rem;">Totale bes�k (7d)</div>
        <div style="color: var(--ink); font-weight: 600; font-size: 1.2rem;">${verifiableVisits.length}</div>
      </div>
      <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
        <div style="color: var(--muted); font-size: 0.85rem;">Bes�kt smittet</div>
        <div style="color: ${confirmedInfectedCount > 0 ? '#dc2626' : '#22c55e'}; font-weight: 600; font-size: 1.2rem;">${confirmedInfectedCount > 0 ? 'YES' : 'NO'}</div>
      </div>
      <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
        <div style="color: var(--muted); font-size: 0.85rem;">48t kontaktkjede</div>
        <div style="color: ${hasContactChain ? '#f97316' : '#6b7280'}; font-weight: 600; font-size: 1.2rem;">${hasContactChain ? 'YES' : 'NO'}</div>
      </div>
    </div>
    ${breachNarrativeHTML}
    ${sameZoneNarrativeHTML}
    ${timelineHTML}
    ${allVisitsHTML}
  `;
...
async function loadVessels() {
  ...
  const getNumberInputValue = (id, fallback) => {
  ...
  const limit = getNumberInputValue("vesselLimit", 100);
  const minSpeed = getNumberInputValue("vesselSpeed", 0);
  const latMin = getNumberInputValue("vesselLatMin", -90);
  const latMax = getNumberInputValue("vesselLatMax", 90);
  const lonMin = getNumberInputValue("vesselLonMin", -180);
  const lonMax = getNumberInputValue("vesselLonMax", 180);
...
function setupListeners() {
  ...
  console.log("[LISTENERS] Available buttons:", {
  ...
  const safeAttach = (element, event, handler, debugName) => {
    if (element && typeof element.addEventListener === 'function') {
      ...
      console.warn(`[LISTENERS] ✗ Failed to attach ${event} to ${debugName}`, element);
      return false;
    }
  };
  ...
  if (typeof coreLoadBindingsModule.setupCoreLoadBindings === "function") {
    try {
      ...
      safeAttach(document.getElementById("admin-load"), "click", () => { loadAdmin(); }, "admin-load");
      safeAttach(document.getElementById("admin-refresh"), "click", () => {loadAdmin(); }, "admin-refresh");

      safeAttach(elements.loadRisk, "click", () => {loadRisk(); }, "loadRisk");
      safeAttach(document.getElementById("risk-load"), "click", () => { loadRisk(); }, "risk-load");

      safeAttach(elements.loadFacilityRisk, "click", () => {loadFacilityRisk(); }, "loadFacilityRisk");
      safeAttach(document.getElementById("facility-risk-load"), "click", () => { loadFacilityRisk(); }, "facility-risk-load");

      safeAttach(elements.loadVesselRisk, "click", () => {loadVesselRisk(); }, "loadVesselRisk");
      safeAttach(document.getElementById("vessel-risk-load"), "click", () => { loadVesselRisk(); }, "vessel-risk-load");

      safeAttach(document.getElementById("predictionsLoad"), "click", () => {loadPredictions(); }, "predictionsLoad");
      safeAttach(document.getElementById("vessel-clearing-load"), "click", () => {loadVesselClearing(); }, "vessel-clearing-load");
      safeAttach(document.getElementById("confirmed-plans-load"), "click", () => {loadConfirmedPlans(); }, "confirmed-plans-load");
      safeAttach(document.getElementById("audit-load"), "click", () => {loadAuditLog(); }, "audit-load");
      safeAttach(document.getElementById("facilities-load"), "click", () => {loadFacilities(); }, "facilities-load");
      safeAttach(document.getElementById("vessels-load"), "click", () => {loadVessels(); }, "vessels-load");
      safeAttach(document.getElementById("ocean-load"), "click", () => {loadOcean(); }, "ocean-load");
      safeAttach(document.getElementById("smitte-load"), "click", () => {loadSmittespredning(); }, "smitte-load");
      safeAttach(document.getElementById("health-load"), "click", () => {loadHealth(); }, "health-load");
  ...
  if (typeof adminActionsModule.setupAdminActions === "function") {
    try {
      ...
      safeAttach(adminActionExport, "click", () => {
        alert("📄 EXPORT: Compliance report ready for download. (Coming soon)");
      });
    }
  } else {
    const adminActionIsolate = document.getElementById("adminActionIsolate");
      ...
      if (priorityList) {
        safeAttach(priorityList, "click", (event) => {
          ...
          if (button.dataset.action === "notify") {
            alert(`📧 Notify: Sending alert to ${link.facility_name} regarding ${link.vessel_name}.`);
          }
        });
      }
    }
  ...
  if (typeof modalEventsModule.setupModalEvents === "function") {
    try {
      ...
      safeAttach(isolateConfirm, "click", () => {
        ...
        alert("✅ Isolation queued: High-risk vessels marked as pending.");
      });
      ...
      safeAttach(approveConfirm, "click", () => {
        ...
        alert("✅ Approval queued: Cleared vessels marked as approved.");
      });
...
async function submitEventForm() {
  ...
  try {
    ...
    if (response.event_id || response.id) {
      alert(`✓ Event created successfully (ID: ${response.event_id || response.id})`);
      closeEventModal();
...
function setAdminIdentityDisplay() {
  ...
  if (userName) {
    ...
    userName.title = "Admin (full tilgang)";
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  if (!window.__kystDashboardInitialized) {
    ...
    setupTabs();
    setupListeners();
    setAdminIdentityDisplay();
    loadOverview();
    loadDecisionLine();  // Load decision line for "What should I do now?" section
  ...
  setupLiveSearch('predictionsSearch', '#predictions-table', [1, 2]); // Søk i anlegg og score
  setupLiveSearch('facilityRiskSearch', '#facility-risk-table', [0, 1]); // Søk i anlegg og kommune
  setupLiveSearch('vesselRiskSearch', '#vessel-risk-table', [0, 1]); // Søk i MMSI og båtnavn
  ...
  function setupSortableColumns() {
    ...
    sortableHeaders.forEach(header => {
      header.addEventListener('click', function() {
        ...
        if (arrow) {
          if (newDir === 'asc') arrow.textContent = '↑';
          else if (newDir === 'desc') arrow.textContent = '↓';
  ...
  function updateInsightsPanel() {
    ...
    if (vesselsAtInfected > 10) {
      criticalAlerts.push(`${vesselsAtInfected} båter har besøkt smittede anlegg`);
    }
    if (activeChains > 3) {
