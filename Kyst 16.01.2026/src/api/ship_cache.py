...
SHIP_CACHE_FILE = "ship_names_cache.json"


class ShipNameCache:
    """Simple in-memory cache for ship names fetched from AIS or other sources."""
    ...
    def _load_cache(self) -> Dict[str, str]:
        ...
        if os.path.exists(SHIP_CACHE_FILE):
            try:
...
def get_ship_name(mmsi: str) -> Optional[str]:
    ...
    return _ship_cache.get_name(mmsi) or _ship_cache.lookup_online(mmsi)
