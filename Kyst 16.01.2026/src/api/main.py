...
import sys
import os
# Legg src-mappen på sys.path uansett hvor du kjører fra
...
# Serve static files from workspace root (where frontend.html and index.html are located)
workspace_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../'))
static_files = ["frontend.html", "index.html"]
app.mount("/static", StaticFiles(directory=workspace_root), name="static")

# Serve frontend.html at root (http://localhost:8000/)
@app.get("/", response_class=HTMLResponse)
async def serve_frontend():
    frontend_path = os.path.join(workspace_root, "frontend.html")
...
async def assess_risk():
    ...
    try:
        ...
        assessments_json = []
        ocean_currents_used = False
        for assessment in assessments:
            # Sjekk om ocean_currents_used er satt på RiskEngine
            if hasattr(engine, 'ocean_currents_used') and engine.ocean_currents_used:
                ocean_currents_used = True
            assessment_json = {
        ...
        return {
            ...
            "ocean_currents_used": ocean_currents_used,
