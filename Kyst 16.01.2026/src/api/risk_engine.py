...
class RiskEngine:
    # Kan settes via miljøvariabel eller kode
    def __init__(self, facilities_data: List[Dict[str, Any]]):
        self.USE_OCEAN_CURRENTS = True
        self.facilities = facilities_data
        self.facilities_by_code = {f["locality"]["no"]: f for f in facilities_data}
        self.bw_client = BarentsWatchClient()  # Initialize BarentsWatch client for ocean data
    ...
    def score_water_exchange(self, latitude: float, longitude: float) -> Optional[float]:
        """Score based on water exchange/current velocity from Copernicus CSV (15% weight). Robust fallback hvis data mangler eller er slått av."""
        import pandas as pd
        import numpy as np
        import os
        if not self.USE_OCEAN_CURRENTS:
            self.ocean_currents_used = False
            return None
        ...
        if df.empty:
            self.ocean_currents_used = False
            return None
        # Finn nærmeste gridpunkt
        df['dist'] = np.sqrt((df['latitude'] - latitude)**2 + (df['longitude'] - longitude)**2)
        ...
        if pd.isna(uo) or pd.isna(vo):
            self.ocean_currents_used = False
            return None
        velocity = np.sqrt(uo**2 + vo**2)
        score = min(velocity * 100, 100)
        self.ocean_currents_used = True
        return score
    ...
    def analyze_vessel_exposure(self, latitude: float, longitude: float, 
                               facility_name: str = "") -> Optional[Dict[str, Any]]:
        """Analyze which vessels are visiting a farm location.
        
        Uses AIS data to identify vessel traffic near the farm.
        Important for understanding disease exposure vectors.
        
        Args:
            latitude: Farm latitude
            longitude: Farm longitude
            facility_name: Name for reporting
            
        Returns:
            Dictionary with vessel analysis or None if no data
        """
        try:
            # Query BarentsWatch for vessels near this location
            vessels = self.bw_client.get_vessels_at_location(latitude, longitude, radius_km=10)
            ...
            return {
                ...
                "note": f"Identified {len(vessels)} vessels within 10km of farm"
            }
            
        except Exception as e:
            print(f"Error analyzing vessel exposure: {e}")
            return None

    def trace_vessel_movement(self, mmsi: int) -> Optional[Dict[str, Any]]:
        """Trace a specific vessel's 7-day movement history.
        
        Returns the complete track showing where the vessel has been.
        Used to identify if vessel visited multiple farms or diseased areas.
        
        Args:
            mmsi: Maritime Mobile Service Identifier (vessel ID)
            
        Returns:
            Dictionary with vessel track and path analysis or None
        """
        try:
            track_data = self.bw_client.get_vessel_track(mmsi, days=7)
            
            if not track_data:
                return None
            
            # Analyze track for distance traveled
            positions = track_data.get("positions", [])
            if len(positions) < 2:
                return track_data
            
            # Calculate total distance traveled (simple Euclidean)
            total_distance = 0
            ...
            track_data["total_distance_km"] = total_distance
            track_data["position_count"] = len(positions)
            
            return track_data
            
        except Exception as e:
            print(f"Error tracing vessel {mmsi}: {e}")
            return None
